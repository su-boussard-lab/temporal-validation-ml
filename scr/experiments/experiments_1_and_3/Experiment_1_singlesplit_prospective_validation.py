...
def fit_models_predict(
    ...
    Args:
        ...
        Indicator whether weights are used for training: weight
    Returns:
        Results template with results appended
    """
    ...
    results_dir_prediction_performance = os.path.join(
        project_root, "results/performance_models/experiment_1/", experiment_name
    )
...
def main(random_state: int = seed.current_seed) -> None:
    ...
    # Equal sampling of train data from each year, testing on full test data
    sample_train = False
    # n_train = 0
    ...
    # -------------- Experiment 1a-b: Single Split with Evaluation on retrospective and prospective test set
    experiment_name = (
        f"Exp_1a_b_SingleSplit_{imputation_type}_1018_1922_s{seed.current_seed}"
    )
    ...
    # -------------- Experiment 1a-b: Single Split with Evaluation on retrospective and prospective test set
    experiment_name = (
        f"Exp_1c_SingleSplit_{imputation_type}_1922_1922_s{seed.current_seed}"
    )
    ...
    experiment_name = (
        f"Exp_1d_SingleSplit_{imputation_type}_1022_1022_s{seed.current_seed}"
    )
    train_years = [[2010, 2022]]
    test_years = [[2010, 2022]]
